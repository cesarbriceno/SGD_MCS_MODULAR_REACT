 ...
 */
function buildCertificateEmailHtml(recipientName, eventName, role) {
            ...
            <tr>
              <td style="padding:40px;">
                ...
                <p style="color:#94a3b8;font-size:12px;line-height:1.5;margin:0;">
                  Este documento tiene validez institucional. Si tienes alguna duda, puedes comunicarte con la coordinación del programa.
                </p>
              </td>
            </tr>
...
function sendSingleCertificateEmail(emailData) {
    try {
        ...
        var htmlBody = buildCertificateEmailHtml(recipientName, eventName, role);

        // 4. Enviar email
        GmailApp.sendEmail(recipientEmail,
            '📜 Certificado Oficial: ' + eventName,
...
function sendBulkCertificateEmails(eventId, certificates) {
    try {
        ...
        if (emailQuotaRemaining < certificates.length) {
            return {
                success: false,
                message: 'Cuota de correo insuficiente. Quedan ' + emailQuotaRemaining + ' correos disponibles hoy, pero se necesitan ' + certificates.length + '.',
                quotaRemaining: emailQuotaRemaining
            };
        }
        ...
        for (var i = 0; i < certificates.length; i++) {
            ...
            try {
                ...
                var htmlBody = buildCertificateEmailHtml(cert.name || 'Participante', currentEventName, cert.role || 'Asistente');

                GmailApp.sendEmail(cert.email,
                    '📜 Certificado Oficial: ' + currentEventName,
