...
import {
    ...
    AlertCircle, ArrowLeft, Download, Plus, Save, Users, Check, Trash2, Filter, Eye, EyeOff, Globe
} from 'lucide-react';
...
const ExternImport = () => {
    ...
    React.useEffect(() => {
        const fetchExisting = async () => {
            try {
                ...
                setExistingExterns(data || []);
            } catch (e) { console.error("Error fetching externs", e); }
        };
        fetchExisting();
    }, []);
    ...
    const validateData = (data) => {
        ...
        const validated = data.map((row, idx) => {
            ...
            if (doc && existingExterns.some(ex => String(ex.Numero_Documento || ex.Cedula) === doc)) {
                rowErrors.push(`El colaborador ya existe(Documento ${doc})`);
            }
            if (email && existingExterns.some(ex => String(ex.Email || "").toLowerCase() === email)) {
                rowErrors.push(`Email ya registrado para colaborador(${email})`);
            }
        ...
        if (report.invalid > 0) {
            report.recs.push("🚨 Registros con observaciones detectados. Depura la lista para proceder.");
            report.recs.push("💡 El sistema sincroniza con la base de datos para evitar duplicidad de registros.");
        } else {
            report.recs.push("✅ ¡Validación superada! Todos los aliados externos son nuevos y válidos.");
            report.recs.push("🆔 Se han asignado códigos de identificación institucional automáticamente.");
        }
            ...
            try {
                ...
                let headerIdx = raw.findIndex(row =>
                    row.length > 2 && row.some(c => {
                        ...
                        return s.includes('nombre') || s.includes('apellido') || s.includes('documento') || s.includes('organizacion') || s.includes('cargo');
                    })
                );
    ...
    const handleImport = async () => {
        ...
        if (validRows.length === 0) return toast.error("Error", "No hay registros listos.");
        ...
        for (let i = 0; i < validRows.length; i++) {
            ...
            ALL_FIELDS.forEach(f => {
                if (rawRecord[f] !== undefined) {
                    if (f === 'Fecha_Ingreso' || f === 'Fecha_Expedicion') {
    ...
    const downloadTemplate = () => {
        ...
        XLSX.writeFile(wb, "Plantilla_Externos.xlsx");
    };
    ...
    return (
        <div className="animate-fade-in pb-32 pt-6 px-4 md:px-8 max-w-[1600px] mx-auto relative z-10">
            ...
            <div className="flex items-center justify-between mb-8 relative z-10">
                <button onClick={() => navigate('/externals')} className="flex items-center gap-2 px-4 py-2 rounded-xl glass-card font-bold text-sm hover:bg-white/60 transition-all shadow-sm">
                ...
                <div className="text-center">
                    <h1 className="text-3xl font-black text-slate-800 dark:text-white flex items-center gap-3">
                        ...
                        Registro Externo Masivo
                    </h1>
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">Colaboradores y Aliados • SGD MCS</p>
                </div>
                <div className="w-24"></div>
            </div>

            <div className={`glass-card p-10 text-center border-2 border-dashed transition-all duration-500 ${file ? 'border-green-500 bg-green-50/5' : 'border-slate-200 dark:border-white/10 hover:border-green-400'}`}>
                {!file ? (
                    <div className="space-y-6">
                        <div className="w-20 h-20 bg-green-100 dark:bg-green-900/30 rounded-3xl flex items-center justify-center mx-auto text-green-600 shadow-inner">
                            <Upload size={36} strokeWidth={2.5} />
                        </div>
                        <div>
                            <h2 className="text-xl font-black text-slate-800 dark:text-white">Cargar Aliados Externos</h2>
                            <p className="text-slate-500 text-sm mt-1">Asegúrate de incluir la Organización y el Cargo de cada persona</p>
                        </div>
                        <div className="flex justify-center gap-4">
                            <label className="px-8 py-3 bg-green-600 text-white rounded-2xl font-black cursor-pointer shadow-xl shadow-green-500/20 flex items-center gap-3 hover:bg-green-500 transition-all transform hover:scale-105 active:scale-95">
                                <Plus size={20} strokeWidth={3} /> Adjuntar Archivo
                    ...
                    <div className="space-y-8 animate-in zoom-in-95 duration-300">
                        <div className="flex items-center justify-center gap-6">
                            ...
                            <div className="text-left">
                                ...
                                <p className="text-slate-400 text-xs font-bold uppercase tracking-wider">Archivo verificado y listo para depurar</p>
                            </div>
                        </div>

                        <div className="flex justify-center gap-12 py-4">
                            <div className="text-center group">
                                <p className="text-[10px] font-black text-slate-400 uppercase mb-1">Entradas</p>
                                <p className="text-3xl font-black text-slate-700 dark:text-white">{validationReport?.total}</p>
                            </div>
                            <div className="text-center group">
                                <p className="text-[10px] font-black text-green-400 uppercase mb-1">Aptos</p>
                                <p className="text-3xl font-black text-green-600">{validationReport?.valid}</p>
                            </div>
                            <div className="text-center group">
                                <p className="text-[10px] font-black text-red-100 bg-red-600 px-2 rounded-full mb-1 italic">Revisar</p>
                        ...
                        {importProgress.status === 'importing' && (
                            <div className="max-w-md mx-auto space-y-2">
                                <div className="h-3 bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden p-0.5 shadow-inner">
                                    <div className="h-full bg-gradient-to-r from-green-500 to-emerald-500 rounded-full transition-all duration-300 shadow-lg"
                                        style={{ width: `${(importProgress.current / importProgress.total) * 100}%` }}></div>
                                </div>
                                <div className="flex justify-between items-center px-1">
                                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Migrando datos externos</p>
                                    <p className="text-xs font-black text-green-600">{Math.round((importProgress.current / importProgress.total) * 100)}%</p>
                                </div>
                            </div>
                        )}
                        ...
                        <div className="flex justify-center gap-4">
                            <button onClick={handleImport} disabled={loading || validationReport?.valid === 0}
                                className="px-10 py-4 bg-green-600 text-white rounded-2xl font-black shadow-xl shadow-green-500/20 disabled:opacity-50 flex items-center gap-3 hover:bg-green-500 transition-all transform hover:scale-105 active:scale-95 uppercase text-xs tracking-widest">
                                {loading ? 'Importando...' : <><Save size={20} strokeWidth={2.5} /> Iniciar Proceso</>}
                            </button>
                            <button onClick={handleCancel} disabled={loading}
                                ...
                                <X size={20} strokeWidth={2.5} /> Descartar
                            </button>
                        </div>
                    </div>
                )}
            ...
            {validationReport && (
                <div className="mt-8 grid grid-cols-1 md:grid-cols-3 gap-6 animate-in slide-in-from-bottom-4 duration-500">
                    <div className={`glass-card p-8 border-l-8 border-l-green-500 shadow-xl ${validationReport.invalid > 0 ? 'md:col-span-2' : 'md:col-span-3'}`}>
                        <div className="flex items-center gap-3 mb-4">
                            <div className="p-2 bg-green-100 dark:bg-green-900/30 rounded-lg text-green-600">
                                <Check size={18} strokeWidth={3} />
                            </div>
                            <h3 className="text-sm font-black uppercase text-slate-800 dark:text-white tracking-widest">Análisis del Colaborador</h3>
                        </div>
                        <ul className="space-y-3">
                            {validationReport.recs.map((r, i) => (
                                <li key={i} className="text-xs font-semibold text-slate-600 dark:text-slate-400 flex items-start gap-4 italic leading-tight">
                                    <div className="mt-1.5 w-1.5 h-1.5 bg-green-500 rounded-full shadow-sm"></div>
                                    {r}
                                </li>
                            ))}
                        </ul>
                    ...
                    {validationReport.invalid > 0 && (
                        <div className="glass-card p-8 border-l-8 border-l-red-500 shadow-xl bg-red-50/10">
                            <div className="flex items-center gap-3 mb-4">
                                ...
                                <h3 className="text-sm font-black uppercase text-red-500 tracking-widest">Datos Inválidos</h3>
                            </div>
                ...
                <div className="mt-12 glass-card overflow-hidden shadow-2xl border-white/40 animate-in fade-in slide-in-from-bottom-8 duration-700">
                    <div className="px-8 py-6 bg-white/40 dark:bg-slate-900/40 backdrop-blur-xl flex justify-between items-center border-b border-white/20 sticky top-0 z-20">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <h3 className="text-sm font-black uppercase tracking-widest text-slate-800 dark:text-white">Depuración de Externos</h3>
                                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-tighter mt-0.5">Limpia y organiza los datos antes del registro masivo</p>
                            </div>
                        </div>
                        <button
                        ...
                        >
                            {showOnlyErrors ? <><EyeOff size={14} /> Viendo Alertados</> : <><Filter size={14} /> Filtrar Alertados</>}
                        </button>
                    </div>
                    ...
                    <div className="overflow-x-auto max-h-[600px] custom-scrollbar table-container">
                        <table className="w-full text-left text-[11px] border-collapse tabular-nums">
                            <thead className="bg-slate-800/10 dark:bg-black/40 text-slate-400 font-black uppercase sticky top-0 z-10 backdrop-blur-md">
                                <tr>
                                    <th className="px-6 py-5 border-b border-white/10 w-16 italic">#</th>
                                    ...
                                    {tableHeaders.map((h, i) => (
                                        <th key={i} className="px-6 py-5 border-b border-white/10 min-w-[150px] group relative">
                                            <div className="flex items-center justify-between">
                                                <span>{h}</span>
                                                <button onClick={() => deleteColumn(h)} title="Quitar columna"
                            ...
                            <tbody className="divide-y divide-slate-200/50 dark:divide-white/5">
                                {filteredData.map((row, i) => (
                                    <tr key={i} className={`group transition-all duration-200 ${row._isValid ? 'hover:bg-green-50/30' : 'bg-red-500/5 hover:bg-red-500/10'}`}>
                                        <td className="px-6 py-4 font-black text-slate-400 opacity-50">{i + 1}</td>
                                        <td className="px-6 py-4">
                                            <div className="flex justify-center text-center">
                                                {row._isValid ? (
                                                    <span className="px-3 py-1 bg-green-100 dark:bg-green-900/30 text-green-600 rounded-full text-[9px] font-black uppercase tracking-tighter flex items-center gap-1">
                                                        <Check size={10} strokeWidth={4} /> Apto
                                                    </span>
                                                ) : (
                    ...
                    {filteredData.length === 0 && (
                        <div className="py-24 text-center space-y-4">
                            ...
                            <p className="font-black text-slate-400 uppercase tracking-widest text-xs">No se encontraron datos críticos</p>
                        </div>
                    )}
                    <div className="px-8 py-5 bg-slate-50/50 dark:bg-black/20 border-t border-white/10 flex justify-between items-center text-[10px] font-black uppercase tracking-widest text-slate-400">
                        <div className="flex gap-10">
                            <span>Viendo: {filteredData.length}</span>
                            <span>Cargados: {previewData.length}</span>
                        </div>
                        <span className="flex items-center gap-2"><Globe size={12} /> {tableHeaders.length} Columnas identificadas</span>
                    </div>
                </div>
            )}
        </div>
